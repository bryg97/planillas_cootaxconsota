<?php
// Configuración de la sesión más segura
session_set_cookie_params([
    'lifetime' => 0,
    'path' => '/',
    'domain' => $_SERVER['HTTP_HOST'],
    'secure' => isset($_SERVER['HTTPS']), // Solo cookies seguras si hay HTTPS
    'httponly' => true,                   // No accesibles desde JS
    'samesite' => 'Strict'                // Evita envío cruzado
]);
session_start();

// Si ya hay una sesión activa, redirigir directamente al panel
if (isset($_SESSION['usuario'])) {
    header("Location: panel.php");
    exit();
}

// Incluir configuración fuera de public_html
include __DIR__ . '/../../config_planillas/config.php';

// Registro seguro de errores (fuera de la vista pública)
